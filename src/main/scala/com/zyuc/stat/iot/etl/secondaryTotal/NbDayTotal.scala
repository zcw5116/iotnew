...
object NbDayTotal {
  def main(args: Array[String]): Unit = {
    ...
    val outputPath = sc.getConf.get("spark.app.outputPath", "/user/iot_ete/data/cdr/summ_d/nbTotal/")
    sqlContext.read.format("orc").load(inputPath + "dayid=" + dayid).registerTempTable("dayTable")
    sqlContext.sql(
      s"""
         |select provid,lanid,eci,apn,tac,busi,industry_form,
         |        sum(sessions) as sessions, sum(upflow) as upflow,
         |        sum(downflow) as downflow,sum(upflow+downflow) as totalFlows
         |from dayTable
         |group by provid,lanid,eci,apn,tac,busi,industry_form
