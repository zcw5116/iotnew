...
/**
  ...
  *
  * 14135 使用3G业务用户清单日统计       epc-log-nm-30
  */
object PdsnDayTotal {
  def main(args: Array[String]): Unit = {
    ...
    val inputPath_3G = sc.getConf.get("spark.app.inputPath_3G", "/user/iot_ete/data/cdr/summ_d/pdsn/")
    val inputPath_4G = sc.getConf.get("spark.app.inputPath_4G", "/user/iot_ete/data/cdr/summ_d/pgw/")
    ...
    val df3G = sqlContext.read.format("orc").load(inputPath_3G + "dayid=" + dayid)
      .filter("service_option='59'").selectExpr("mdn","upflow","downflow")

    val df4G = sqlContext.read.format("orc").load(inputPath_4G + "dayid=" + dayid)
    ...
    val dfUnion = sqlContext.sql(
            s"""
             ...
             """.stripMargin)
    dfUnion.coalesce(20).write.mode(SaveMode.Overwrite).format("orc").save(outputPath + "dayid=" + dayid)
