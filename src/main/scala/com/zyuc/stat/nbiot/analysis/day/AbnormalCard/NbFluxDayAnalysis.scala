...
//       """.stripMargin).coalesce(10).write.format("orc").mode(SaveMode.Overwrite).save(outputPath + "/" + d + "/base")
    ...
    val baseFluxDF = baseFlux.selectExpr("custid", "custname", "own_provid", "own_lanid", "'NB' as netType",
...
//                                                  "'-1' as upPacket", "'-1' as downPacket", "'-1' as totalPacket")
    ...
    insertAbnormalByJDBC(outputPath + "/" + dayid + "/abnormalFlux", tableName, tableName+"_flux_nb")
