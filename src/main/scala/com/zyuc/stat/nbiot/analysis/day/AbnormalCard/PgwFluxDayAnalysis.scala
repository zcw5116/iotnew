...
object PgwFluxDayAnalysis {
  def main(args: Array[String]): Unit = {
    ...
    val baseFluxDF = baseFlux.selectExpr("custid", "custname", "own_provid", "own_lanid", "'4G' as netType",
    ...
    val abnormalFluxDF = abnormalFlux.selectExpr("'DAY' as gather_cycle", s"'${dayid}' as gather_date",
      ...
      "'4G' as net_type", "custid", "custname",  "mdn",
    ...
    //异常卡流量保存到hdfs
    abnormalFluxDF.coalesce(10).write.format("orc").mode(SaveMode.Overwrite).save(outputPath + "/" + dayid + "/abnormalFlux")

    insert3GAbnormalByJDBC(outputPath + "/" + dayid + "/abnormalFlux", tableName, tableName+"_flux_4g")
