...
/**
  ...
  */
object GwMmeDayAnalysis {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf()//.setMaster("local[2]").setAppName("name_20190711")
    ...
    val outputPath = sc.getConf.get("spark.app.outputPath","/user/epciot/data/neimeng/summ_d/")
    ...
    try{
      ...
      df = sqlContext.sql(
              s"""
                 ...
                 |       '$dataTime' stattime, '内蒙古' provflag, 'day' timeflag
                 |from ${table_sgw} a
