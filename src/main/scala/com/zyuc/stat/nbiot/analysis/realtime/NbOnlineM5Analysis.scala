...
object NbOnlineM5Analysis {
  def main(args: Array[String]): Unit = {
    ...
    val inputPath = sc.getConf.get("spark.app.inputPath", "/user/iot/data/online/transform/nb/data")
    val outputPath = sc.getConf.get("spark.app.outputPath", "/user/iot/data/online/analy_realtime/nb")
    ...
    val statDF = sqlContext.sql(
      s"""
         |select custid, nvl(prov, '-') as prov, nvl(city, '-') as city, num from ${tmpTable}
       """.stripMargin)
    ...
    val sql =
      s"""
         |insert into iot_ana_5min_nb_online_20${d}
    ...
    // 更新断点时间
    CommonUtils.updateBreakTable("iot_ana_5min_nb_online", dataTime+"00")
  }
}
