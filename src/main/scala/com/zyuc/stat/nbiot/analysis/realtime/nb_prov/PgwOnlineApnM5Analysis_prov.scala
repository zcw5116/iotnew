...
/**
  ...
  */
object PgwOnlineApnM5Analysis_prov {
  def main(args: Array[String]): Unit = {
    val sparkConf = new SparkConf()//.setMaster("local[2]").setAppName("PgwOnlineApnM5Analysisprov_201810101010)
    val sc = new SparkContext(sparkConf)
    val sqlContext = new HiveContext(sc)

    val appName = sc.getConf.get("spark.app.name","PgwOnlineApnM5Analysisprov_201810101010")
    val inputPath = sc.getConf.get("spark.app.inputPath", "/user/iot/data/online_apn_prov/transform/4g/data")
    val outputPath = sc.getConf.get("spark.app.outputPath", "/user/iot/data/online_apn_prov/analy_realtime/4g")
    ...
    CommonUtils.updateBreakTable("iot_ana_5min_prov_4g_stat_online", dataTime+"00")
